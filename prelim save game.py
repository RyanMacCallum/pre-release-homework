...
class Breakthrough():
    ...
    def save_to_game_2(self):#
        with open("game2.txt", "w") as saved_game:
            ...
            saved_game.write(f"{self.__CurrentLock.lock_solved_status()}\n")
            saved_game.write(f"{self.__Hand.GetCards()}\n")
            saved_game.write(f"{self.__Sequence.GetCards()}\n")
            saved_game.write(f"{self.__Discard.GetCards()}\n")
            saved_game.write(f"{self.__Deck.GetCards()}")
...
class Lock():
    ...
    def save_lock_string(self):
        challenges_as_string = ''
        for challenge in self._Challenges:
            challenges_as_string += ",".join(challenge.GetCondition()) + ";"
        challenges_as_string = challenges_as_string[:-1]
        return challenges_as_string

    def lock_solved_status(self):
        status_as_string = ''
        for challenge in self._Challenges:
            if challenge.GetMet():
                status_as_string += "Y" + ";"
                ...
                status_as_string += "N" + ";"
        status_as_string = status_as_string[:-1]
...
class CardCollection():
    ...
    def GetCards(self):
        ...
        for card in self._Cards:
            ...
            cards += str(description) + " " + str(number) + "," + " "
        cards = cards[:-1]
        return cards
